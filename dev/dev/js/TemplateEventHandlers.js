...
import { functionalityConfig } from './TemplateConfig.js';
...
export function updateLeftSidebar(functionalityKey, subClassification) {
    ...
    console.log("Inside update Left sidebar ", functionalityKey, subClassification);
    const config = functionalityConfig[functionalityKey];
    ...
    let content = leftSidebarContent[subClassification]; 

    console.log("Extracted content for left sidebar:1", content);
 // Get specific content

    // ✅ FIX: Ensure we only assign strings, not objects
    if (functionalityKey === "trigonoRatios") 
        content = leftSidebarContent;  // Pick a string
    

    console.log("Extracted content for left sidebar:2", content);
    ...
    // ✅ Now we are sure content is a string
