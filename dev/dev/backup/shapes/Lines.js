...
export class Line extends Shape {
    ...
    */
    ...
    drag(dx, dy) {
        console.log(`Dragging ${this.constructor.name}: dx = ${dx}, dy = ${dy}`);
    if (this.draggingPoint) {
        console.log(`Dragging endpoint: (${this.draggingPoint.x}, ${this.draggingPoint.y})`);
        this.draggingPoint.drag(dx, dy);
    } else {
        console.log('Dragging the entire line');
        this.start.drag(dx, dy);
        this.end.drag(dx, dy);
    }
}
    ...
    static create(x1, y1, x2, y2, color=black, labelStart = null, labelEnd = null) {
