    ...
    export class Triangle extends Shape {
        ...
        setMidpointLabels(midpointLabels) {
            ...
            if (!midpointLabels || typeof midpointLabels !== 'object') return;
        
            // Make sure midpoints are calculated before applying labels
            if (this.midpoints.length === 0) {
                this.calculateMidpoints();
            }
        
            if (!this.midpoints || this.midpoints.length < 3) {
                console.warn("⚠️ Midpoints missing or incomplete in Triangle object.");
                return;
            }
        
            this.midpoints.forEach((midpoint, index) => {
                const labelKey = `M${index + 1}`;
                if (midpoint && midpointLabels[labelKey]) {
                    midpoint.label = midpointLabels[labelKey];
                }
            });
        }
        ...
        draw(ctx) {
            ...
            if (this.showMeasurements) this.drawSideLengths(ctx);
            if (this.showLabels) {
                this.points.forEach((point) => point.draw(ctx, true));
            }
            this.drawExtensions(ctx);
            if (canvasManager.showMeasurements && this.measurements.angles) this.drawInternalArcs(ctx);
            if (canvasManager.showMeasurements && this.measurements.sides) this.drawSideLengths(ctx);
            if (canvasManager.showMeasurements && this.measurements.area) this.drawArea(ctx);
